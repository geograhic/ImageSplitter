...
// Web 版改造：原 Electron 桌面端通过 vendor/jszip.min.js 以全局 <script> 注入 JSZip。
// 现在作为 ES module 直接 import（由 Vite 打包），不再依赖本地 vendor 文件，也不依赖 Node.js。
...
// （ES module 作用域不是全局，内联事件处理器找不到模块内函数，需显式挂到 window）
// =============================================
window.$ = $;
window.switchTab = switchTab;
window.batchExportAll = batchExportAll;
...
window.toggleGuidesVisibility = toggleGuidesVisibility;
window.renderPresetManagerModal = renderPresetManagerModal;
